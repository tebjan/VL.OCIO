...
function toGPUSettings(s: PipelineSettings, viewExposure: number, effectiveInputSpace: number): GPUPipelineSettings {
  const g = s.colorGradeEnabled !== false; // neutral bypass when grading disabled
  return {
    inputSpace: effectiveInputSpace,
    gradingSpace: s.gradingSpace,
    gradeExposure: g ? s.gradeExposure : 0,
    contrast: g ? s.gradeContrast : 1,
    saturation: g ? s.gradeSaturation : 1,
    temperature: g ? s.gradeTemperature : 0,
    tint: g ? s.gradeTint : 0,
    highlights: g ? s.gradeHighlights : 0,
    shadows: g ? s.gradeShadows : 0,
    vibrance: g ? s.gradeVibrance : 0,
    lift: g ? [s.gradeLift.x, s.gradeLift.y, s.gradeLift.z] : [0, 0, 0],
    gamma: g ? [s.gradeGamma.x, s.gradeGamma.y, s.gradeGamma.z] : [1, 1, 1],
    gain: g ? [s.gradeGain.x, s.gradeGain.y, s.gradeGain.z] : [1, 1, 1],
    offset: g ? [s.gradeOffset.x, s.gradeOffset.y, s.gradeOffset.z] : [0, 0, 0],
    shadowColor: g ? [s.gradeShadowColor.x, s.gradeShadowColor.y, s.gradeShadowColor.z] : [0, 0, 0],
    midtoneColor: g ? [s.gradeMidtoneColor.x, s.gradeMidtoneColor.y, s.gradeMidtoneColor.z] : [0, 0, 0],
    highlightColor: g ? [s.gradeHighlightColor.x, s.gradeHighlightColor.y, s.gradeHighlightColor.z] : [0, 0, 0],
    highlightSoftClip: g ? s.gradeHighlightSoftClip : 0,
    shadowSoftClip: g ? s.gradeShadowSoftClip : 0,
    highlightKnee: g ? s.gradeHighlightKnee : 1,
    shadowKnee: g ? s.gradeShadowKnee : 0.1,
