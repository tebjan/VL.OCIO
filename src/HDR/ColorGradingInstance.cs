...
{
    private const string Tag = "ColorGradingInstance";
    ...
    // Cached JSON output with dirty tracking
    ...
    public ColorGradingInstance(NodeContext nodeContext, string settings = "")
    {
        ...
        GradeLog.Debug(Tag, $"Created '{_autoGeneratedId}' id={_stableId} slice={_sliceIndex} (exported: {_isExported})");
    }
    ...
    /// <param name="currentSettings">Output: current effective settings as a JSON string</param>
    /// <param name="error">Output: error message if something fails (empty when ok)</param>
    ...
    /// <param name="settingsJson">JSON string to load as new settings (use with Apply Settings)</param>
    /// <param name="applySettings">When true, parses the Settings JSON and applies it to this instance</param>
    public void Update(
        ...
        bool applySettings = false)
    {
        ...
        if (applySettings && _registered && !string.IsNullOrWhiteSpace(settingsJson))
        {
            ...
            if (parsed != null)
            {
                _service.ApplySettingsFromJson(_stableId, parsed);
                _lastError = "";
            }
            else
            {
                _lastError = "Failed to parse settings JSON";
            }
        }
        ...
        // JSON output with dirty-flag caching
        if (_outputDirty)
        {
            var currentState = new ProjectSettings
            {
                ColorCorrection = outColorCorrection,
                Tonemap = outTonemap,
                InputFilePath = _hasRuntimeState ? (_runtimeInputFilePath ?? "") : _parsedSettings.InputFilePath
            };
            _cachedOutputJson = currentState.ToJson();
            _outputDirty = false;
        }
    ...
    public void Dispose()
    {
        ...
        GradeLog.Debug(Tag, $"Disposed '{_autoGeneratedId}' ({_stableId}) slice={_sliceIndex}");
    }
}
